///改动:
///添加了兼容嵌套UI的版本,注意该版本下,动画执行过程中分辨率若变化,动画结果也会变化
///<see cref="Platinio.TweenEngine.PlatinioTween.MoveUI_Modified(RectTransform, Vector2, RectTransform, float)"/>
///添加了对TMPText支持的重载
///<see cref="Platinio.TweenEngine.PlatinioTween.Fade(TMP_Text, float, float)"/>
///添加了对TMPText支持的重载
///<see cref="Platinio.TweenEngine.PlatinioTween.ColorTween(TMP_Text, Color, float)"/>
...
namespace Platinio.TweenEngine
{
    ...
    public class PlatinioTween : Singleton<PlatinioTween>
    {
        ...
        public BaseTween Fade(TMP_Text text, float to, float t)
        {
            ValueTween tween = TweenPool.GetValueTween(text.color.a, to, t);
            tween.SetOnUpdateFloat(v =>
            {
                if (text == null)
                {
        ...
        public BaseTween MoveUI_Modified(RectTransform rect,Vector2 absolutePosition,RectTransform canvas,float time)
        {
            Vector2 targetPos = Paltry.TransformUtil.UIRelativeCanvasPos2WorldPos(absolutePosition, canvas);

            return VectorTween((Vector3) rect.position,(Vector3) targetPos, time)
                .SetOnUpdateVector3((Vector3 value) =>
            {
                if (rect == null)
                    return;
