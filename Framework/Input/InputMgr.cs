...
namespace Paltry
{
    ...
    public class InputMgr : CSharpSingleton<InputMgr>
    {
        ...
        struct InputThreshold//输入检测的阈值
        {
            ...
            public const float LongPress = 0.3f;
        }
        public InputMgr()
        {
        ...
        private void UpdateKeyState(ref KeyState key)
        {
            ...
            {
                EventCenter.Trigger("Input.KeyDown." + key.keyCode.ToString());
                float delta = RealTimeNow - key.lastUpTime;
                if (key.isSecondClick && delta < InputThreshold.DoubleClick)
                {
                    EventCenter.Trigger("Input.KeyDoubleClick." + key.keyCode.ToString());
            ...
            {
                EventCenter.Trigger("Input.KeyOn." + key.keyCode.ToString());
                float longPressedTime = RealTimeNow - key.lastDownTime;
                if (longPressedTime > InputThreshold.LongPress)
                    EventCenter.Trigger<float>("Input.KeyLongPress." + key.keyCode.ToString()
                        , longPressedTime);
            }
            if (Input.GetKeyUp(key.keyCode))
            {
                EventCenter.Trigger<float>("Input.KeyUp." + key.keyCode.ToString()
        ...
        /// <summary>
        /// 改键功能,最好在外部检测一下改键后是否有按键重复,并取消先前按键事件的监听
        /// </summary>
        public void ResetKey(KeyCode orgKey, KeyCode newKey)
        {
            for (int i = 0; i < KeyStates.Length; i++)
            {
                if (KeyStates[i].keyCode == orgKey)
                {
                    KeyStates[i].keyCode = newKey;
                    return;
                }
            }
        }
        ...
        /// 设置所有按键的状态
        /// </summary>
        public void SetKeyCheck(bool isEnabled)
        {
            for (int i = 0; i < KeyStates.Length; i++)
                KeyStates[i].isDisabled = !isEnabled;
        }
    ...
    public partial struct EventName
    {
        public struct Input
        {
            public struct Down
            {
                ...
                /// Base后面加上任意按键即是该事件
                /// </summary>
                public const string Base = "Input.KeyDown.";
                ...
                public const string Space = "Input.KeyDown.Space";
            }
            public struct On
            {
                public const string Base = "Input.KeyOn.";
            ...
            public struct Up
            {
                public const string Base = "Input.KeyUp.";
            ...
            {
                public const string Base = "Input.KeyDoubleClick.";
            ...
            {
                public const string Base = "Input.KeyLongPress.";
